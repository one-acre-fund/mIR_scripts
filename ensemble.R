...
required.packages <- c("caret","readr","soil.spec","doMC","caretEnsemble",
                       "ggplot2","dplyr", "wavelets","prospectr","gridExtra")
...
## RUN FROM HERE
ENS <- function(wd, infrared.data, reference.data){
    ...
    print(paste("vecton length of testing split is ", length(testing)))
    ...
    for(q in 1:length(hd)){
      ...
      model_list <- caretList(
        ...
        trControl=ctrl,
        tuneList=list(
          pls_tuned=caretModelSpec(method="kernelpls",
                                   tuneGrid = expand.grid(ncomp = 2:5)),
          svm=caretModelSpec(method="svmLinear2",
                             tuneGrid = expand.grid(cost=c(1000,5000,10000))),
          glmnet=caretModelSpec(method="glmnet", 
                                tuneGrid = expand.grid(lambda=(1:10)*0.1, alpha=(1:10)*0.1))
        ))
      ...
      ens_preds <- predict(greedy_ensemble, newdata=testX, type="raw")
      ens_train <- predict(greedy_ensemble, newdata=trainX, type="raw")
      RSQ <- postResample(pred = ens_preds, obs = testY)[2]
      RSQ.train <- postResample(pred = ens_train, obs = trainY)[2]
      
      pm <- as.data.frame(cbind(trainY, round(ens_train), 2))
      ...
      p <- p + annotate('text', label = paste('R^2 == ', RSQ.train), 
      ...
      model.summary <- c(hd[q], RSQ.train, RSQ)
      msummary <- rbind(msummary, model.summary)
      
      #saveRDS(xgb.fit, file = paste0(b,"/","Models/", hd[q], ".rds"))
      predi <- round(ens_preds, 2)
      pmp <- as.data.frame(cbind(testY, predi))
      colnames(pmp) <- c("measured","predicted")
      
      # Create scatter plot for the predicted versus the measured
      # the validation set
      p2 <- ggplot(pmp, aes(x = measured, y = predicted)) + 
      ...
      p2 <- p2 + annotate('text', label = paste('R^2 == ', RSQ), 
                          parse = TRUE,Inf, -Inf,hjust = 2.5, vjust = -7.8) 
    ...
    colnames(msummary) <- c("Soil properties", "In Sample Rsq", "Holdout Rsquared")
    ...
    for(q in 1:length(hd)) {
      ...
      ens_preds <- predict(greedy_ensemble, newdata=trainX, type="raw")
      RSQ <- postResample(pred = ens_preds, obs = trainY)[2]
      ...
      prediction.f <- round(predict(greedy_ensemble, as.matrix(der1.ssn[, -1])), 2)
