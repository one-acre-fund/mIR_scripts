...
registerDoMC(detectCores())         #Max Sop setup - comment out if you don't have an 8-cores machine
#source("misc.R")
#registerDoParallel()
#getDoParWorkers()

calibrate <- function(wd,infrared.data,reference.data,method = c("RF","PLS")){
  ...
  write.table(der1.ssn, file = "First derivative.csv",sep = ",",row.names = FALSE)
    ...
    #der1.ssn<-as.data.frame(read_csv("First derivative.csv"))# what the heck was this line here for?
  ...
  #print("2")
  print(paste("size of testing is ", length(testing)))
  ...
  for (q in 1:length(hd)){
  	...
  	set.seed(1001)
  	ctrl <- trainControl(method = "repeatedcv", index = indx, number = 10, repeats = 3)
  	cor <- cor(trainX)
  	high_cor <- findCorrelation(cor, 0.998)
  	trainX <- trainX[, -high_cor]
  	#print(paste("training set dim ", dim(trainX)))
  	tunegrid <- expand.grid(.mtry=c(37,41,43,45,47,49,51,53)) #, .coefReg=c(0.001,0.01,0.1,0.3)
  	#tunegrid <- expand.grid(n.trees=c(500),interaction.depth=c(9,12),
  	#                        shrinkage = 0.01, n.minobsinnode = c(20)) 
  	set.seed(1001)
  	rf.m <- train(y=trainY , x=trainX, method = "rf",	trControl = ctrl,
    ...
    mtry <- rf.m$finalModel$mtry
    # mtry <- rf.m$finalModel$n.trees
    # t <- rf.m$finalModel$interaction.depth
    # mtry <- paste(mtry,t,sep = "-")
  	training.parameters <- c(hd[q], mtry, round(postResample(predicted, measured), 2))
	...
	gc()
  	...
  	colnames(msummary) <- c("Soil properties", "mtry", "OOB RMSEC","OOB Rsquared",
  	                        "Holdout RMSEP","Holdout Rsquared")
  	
  	write.table(msummary, file = "Model_Summary.csv", sep = ",",
  	            row.names = FALSE)
  	#-----------------------------
  	...
  	colnames(cal) <- c("trainY", colnames(cal[, -1]))
  	
  	#neg <- which(cal[, 1] < 0)
  	#ifelse(neg, print(paste("dropping row with ", cal[neg, 1])), cal <- cal[-neg, ])
  	cal[, 1] <- log(cal[, 1]) 
  	cal <- na.omit(cal)
  	...
  	trainX <- trainX[, -high_cor]
  	#print(paste("training set dim ", dim(trainX)))
  	tunegrid <- expand.grid(.mtry=mtry) #, .coefReg=c(0.001,0.01,0.1,0.3)
  	#tunegrid <- expand.grid(n.trees=c(500),interaction.depth=c(9,12),
  	#                        shrinkage = 0.01, n.minobsinnode = c(20)) 
  	set.seed(1001)
  	rf.m <- train(y=trainY , x=trainX, method = "rf",	trControl = ctrl,
  	...
  	training.parameters <- c(hd[q], mtry, round(postResample(predicted, measured), 3))
  	# Save the full model.
  ...
  colnames(msummary) <- c("Soil properties","mtry","RMSEC","Rsquared")

  #Save full models' summary and predicted values
   
   write.table(msummary, file = "Full_models_summary.csv",
